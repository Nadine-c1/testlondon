...
if pagina == 'Data verkenning':
    ...
    # Streamlit App
    st.title("🚲 Correlatie fietsritten en weer Londen 2019")

    st.title('Weer in Londen')
    ...
    fig.update_layout(template='plotly_dark', height=600, width=1500, barmode='overlay', bargap=0.1, showlegend=True, legend=dict(x=1.05, y=1))
...
#############################################################################
    ...
    st.subheader("📊 Correlatie Heatmap (Fietsen/weer 2019)")
...
#######################################################################################################################################
 ...
 #Hier de temperatuur, neerslag en het fietsgebruik Londen 2019

    #Subset weer 2019
    df_weather_19 = weather[weather['Datum'].dt.year == 2019]

    fig = go.Figure()

    # Plot regenval
    fig.add_trace(go.Scatter(
        x=df_weather_19['Datum'],
        y=df_weather_19['prcp'],
        mode='lines',
        name='regenval in mm',
    ...
    fig.add_trace(go.Bar(
        x=ritten_per_dag_fiets['Start Date'],
        y=ritten_per_dag_fiets['Aantal Ritten'],
    ...
    fig.update_layout(
        title='Temperatuur & Fietsritten in London 2019',
        xaxis_title='Datum',
        yaxis=dict(
    ...
    st.subheader("Weer tegenover fiets data Londen 2019")
    st.plotly_chart(fig, use_container_width=True)
...
#########################################################################################################################
    #Voorspelling proberen in hetzelfde format te krijgen
    ...
    # Layout met tweede y-as
    fig.update_layout(
        title='Voorspelde Fietsritten Jan/Feb 2020',
        xaxis_title='Datum',
        yaxis=dict(
    ...
    # Extra plot: Compare predicted vs actual bike rides
    fig = go.Figure()

    # Plot predicted bike rides (red line)
    fig.add_trace(go.Scatter(
        x=weer_2020_avg['Date'],
        y=weer_2020_avg['Voorspelde Fietsritten'],
        mode='lines',
...
###########################################################################################################################
   ...
   - De gevonden relatie kan worden gebruikt om het aantal fietsritten te voorspellen aan de hand van weersvoorspellingen. Echter zit er wel een significant verschil tussen doordeweekse dagen en weekenddagen, dit is in het voorspelling niet meegenomen.
    """)
